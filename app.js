...
const createSlider = () => {
  ...
  let inputDuration = document.getElementById('doration').value || 1000;

  inputDuration=Math.abs(inputDuration);
  let doration = (inputDuration >= 1000) ? inputDuration : 1000;
...
//add search button eventlistener on press enter
search.addEventListener('keyup', function(press) {
  if (press.key === "Enter") {
    searchBtn.click();
  }
...
const toggleSelectedItem=(img,isadded)=>{
  if (isadded===true) {
